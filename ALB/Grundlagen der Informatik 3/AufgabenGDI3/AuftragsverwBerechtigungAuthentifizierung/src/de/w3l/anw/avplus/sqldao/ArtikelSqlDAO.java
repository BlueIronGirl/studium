...
public class ArtikelSqlDAO extends SqlDAO<ArtikelTO> implements
  ...
  @Override
  public ArtikelTO load(Connection verbindung, ResultSet rs) throws DAOException {
    try {
      ArtikelTO result = new ArtikelTO();
      result.artikelnr = rs.getInt(1);
      result.artikelbezeichnung = rs.getString(2);
      result.preis = new Geld(rs.getLong(3), rs.getInt(4), Currency.getInstance(rs.getString(5)));
  ...
  public void deleteDependendItems(Connection verbindung, int key) throws SQLException {
    // Es gibt keine Depended-Items, daher leere Implementierung
  }

  public Iterator<ArtikelTO> findeKleinerMindestbestand() {
    try {
      ResultSet rs = this.verbindung.createStatement().executeQuery(
