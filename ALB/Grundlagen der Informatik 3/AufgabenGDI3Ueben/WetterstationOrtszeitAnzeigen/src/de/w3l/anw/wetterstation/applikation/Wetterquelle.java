...
public class Wetterquelle
{
  ...
  /**
   * Holt für den angegebenen Ort einen neuen Datensatz. Da
   * Wetterquellen wie z.b. Weather Underground in ihren
   * Lizenzbedingungen eine maximale Anzahl von Abfragen
   * vorsehen, wird die Abfrage entsprechend verzögert, wenn
      ...
      throws WetterquelleException
  {
    ...
    // Jetzt warten wir, bis wir Daten liefern dürfen
    System.out.println("Nächste Wetterdatenabfrage: "
        + letzteAbfrageWetter);
    ...
    // Jetzt dürfen wir wieder Daten liefern
    try
    {
      /*
       * Zunächst muss ein JAXBContext-Objekt erzeugt
       * werden, das die annotierte Klasse kennt. Dies
       * geschieht durch die statische Methode newInstance.
       * Die Klassen, die dem Kontext bekannt sein müssen,
       * werden als Parameter hinzugefügt.
       */
      JAXBContext kontext =
          JAXBContext
       ...
       * erzeugen zunächst einen Unmarshaller für den
      ...
      /*
       * Jetzt kann der Unmarshaller tätig werden. Wir
      ...
      // erfolgt dies jetzt. Um zu überprüfen, ob ein neuer
      // Tag vorliegt, muss getestet werden, ob
      // letzteAbfrageWetter schon einen Tag weiter ist als
      // letzteAbfrageAstronomie.
      // Tatsächlich wird getestet, ob nicht gilt,
      // dassdie Astronomiedaten überhaupt schon einmal
      ...
      if (ortsId != 0)
      {
        ...
        {
          /*
           * Wir haben bereits einen Ort und müssen nun
           * prüfen, ob die ID dieses Ortes mit den
           * eingelesenen Daten übereinstimmt.
           */
          if (!(ort.getId().equals(String.valueOf(ortsId))))
            throw new WetterquelleException(
                "Daten für falschen Ort angefordert");
          // Zeitzone sicherheitshalber setzen, da sich
          // die Differenz zu GMT wegen Sommerzeit/
          // Winterzeit geändert haben könnte.
